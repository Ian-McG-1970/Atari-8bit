...
L0417_DOSINI:      rts			;DOSINI points here
            
L0418_DOSVEC:		sei			;Program jumps here after warm boot - DOSVEC points here
					ldx #$00		;
					stx NMIEN		;Disable NMI's
					dex			;X = $FF
					txs			;Stack = $FF
					jsr L0B5C_INIT		;Writes 0 to $7800 - $78A5, $12 - $F5, $86, $87. Writes 1 to $13C - $143, $F5 - $FC
						;Writes 0 to $D200 - $D208(AUDF1,AUDC1 - AUDF4,AUDC4), $8E - $91 - Other initializations
					jsr L0568_INIT		;Clears memory $7B00 - $7FFF, AUDCTL, $88, $89, and more initalizations
					jmp L046D_TITLES		;Jump to main body of program
...
DemoMode:   sei			;
            ...
            dex			;
            txs			;Stack = $FF(Top)
            jsr L0B5C_INIT		;Clears PM memory, clears sound registers, 4 lives for Bruce, Yin-Yang value, etc
            jsr L0568_INIT		;Game screen and audio initalizations. 		
            lsr L00F8		;
            lda #$F7
            sta PORTB
            lda L398E
            ...
            sta L398E
            jsr L06F3
            jsr L1E45_DRAW		;Draw players, store player color, size
            lda #$00
            sta ICCOMT
            lda #$80
            sta DSKFMS
...
L046D_TITLES:		lda #$F3			;
					sta PORTB			;
            jsr L1E34_AUDIO					;Clears audio again(AUD(C,F)1-4) and writes 0 to $8E - $91
            jsr L1F40_PLAYER_SPRITE_SETUP	;Something to do with Bruce, Yamo, and Ninjas status.
            jsr L13A9_SCREEN_CLEAR			;Writes 0 to $78A9 - $7B19 using pointers($39D9 - $39F5) - Again
            jsr L0A91_TTL_COL	;Puts title screen colors in play field registers. 
            lda #$08			;
...
L048A:      lda L0AA6,X			;Moves 4 bytes - $EA, $4E, $4A, $4A
            ...
            ldx #$00			;
            ldy #$00			;
            jsr L11E2_DRW_SCN	;This copies bytes to screen memory locations
            ...
            ldy #$18			;
            jsr L11E2_DRW_SCN	;More title screen Calculations
            lda #$09			;
            ldx #$00			;
            ldy #$2C			;
            jsr L11E2_DRW_SCN	;More title screen Calculations
...
L04F2_LOOP		lda L7900,X		;
				...
				sta L7930,X		;
				dex				;
				bpl L04F2_LOOP	;Inverse till line done
...
L0519:      lda L398F			;Get Comp/Opp status
            ...
            jsr L0AAA_FLPCHR	;If Opp flip characters to inverse
L0521:      lda #$09			;
            sta FMSZPG+4		;$47
            lda #$60			;
            sta FMSZPG+3		;$46
...
;Start key or jostick button(s) were pressed
            ...
            txs					;Stack = $FF(Top)
            jsr L0B5C_INIT		;Clears PM memory, clears sound registers, 4 lives for Bruce, Yin-Yang value, etc
            jsr L0568_INIT		;Game screen and audio initalizations. See Below
...
L056B_LOOP:    	sta L7C00,X			;$7B00 - $7FFF = 0
            ...
            lda #$FF			;
            sta PORTB			;
            jsr L0614_SET_INT	;This subroutine sets up a Disp. List, DLI, VBI, PMBASE, CHBASE, and Enables IRQ's
            lda #$03			;
            ...
            jsr L066B_SET_COLOURS	;Set playfield colors
            ldy #$78			;
            lda #$A9			;
            jsr L064C			;Computes pointer addresses($78A9 - $7B19) and stores them at $39D9 - $39F5
            jsr L13A9_SCREEN_CLEAR			;Writes 0 to $78A9 - $7B19 using pointers($39D9 - $39F5). Memory locations for the Score/lives line
            lda #$80			;
            ldx #$03			;
L05B7_LOOP:   	sta L78A5,X			;Store $80 to $78A5 - $78A8 - Black spaces before Score line
				dex					;$80 tells display routine when to stop copying screen data
				bpl L05B7_LOOP	;
            ldx #$07			;
L05BF_LOOP:    	sta L78D1,X			;Store $80 to $78D1 - $78D8
				dex					;
				bpl L05BF_LOOP		;
            ldx #$03			;
L05C7_LOOP:    	sta L7901,X			;Store $80 to $7901 - $7904
				dex					;
				bpl L05C7_LOOP		;
            ...
            jsr L13CC_1UP		;Draws '1UP' to screen
            ...
            ldx #$04			;
            lda #$FF			;
L05EE_LOOP:    	sta L7801,X		;Loop and store $FF at $7804, $7803, $7802, $7801
				dex				;
				bpl L05EE_LOOP	;
            ldx #$00			;
            lda #$00			;
...
L0603_LOOP:    	lda VCOUNT		;-------------------------------------
				cmp #$08		;Wait until screen beam is in between
				bcc L0603_LOOP	;Line 8 and line $50 before enabling
				cmp #$50		;DMA
				bcs L0603_LOOP	;
            lda #$3F			;
            sta DMACLT			;--------------------------------------
...
L0614_SET_INT: lda #<L0F6E_DLI1 	; #$6E			;
            sta VDSLST			;
            lda #>L0F6E_DLI1 	; #$0F			;
...
L066B_SET_COLOURS:      ldx #$04			;Loop 4 times
L066D:      				lda L3E6A_COLOURS,X			;Data area
...
L06A0:      lda L398F			;Load value
            ...
            sta L3991			;
            jsr L0AAA_FLPCHR	;Flips between 'COMPUTER - OPPONENT'	
...
L0705:      lda #$20			;
            ...
            sta L0EF6			;????
            jsr L38FF			;????
            jsr L1E34_AUDIO		;Writes 0 to $D200 - $D208(AUDC1,AUDF1 - AUDC4,AUDF4) and $8E - $91.
...
L07D1:      lda (BUFADR),Y			;
            ...
            sta BUFADR+1
            ldx #$02
            jsr L13B7_SET_RTCLOK				;This subroutine moves low and high byte to $13, $14 (RTCLOK+1, RTCLOK+2)
            lda RTCLOK+1
            sec
            sbc #$04
            sta RTCLOK+1
...
L0830:      jsr L2ACA				;Finished with loop will jump here
            jmp L318F_SCR_NO		;Returns to an address based on screen number
...
L0972_SET_YAMO: lda DINDEX			;Yamo spawn timer
...
L0A07_TIMER_CHK: lda COLCRS+1		;Load spawn timer for Ninja
...
L0A14_SET_NINJA: lda COLCRS+1		;Load Ninja spawn timer
...
L0AAA_FLPCHR: ldx #$13				;Number of characters to process
...
L0B05:      lda #$00				;
            ...
            jsr L13A9_SCREEN_CLEAR				;Clears the screen
            jsr L1E34_AUDIO			;Clears the Audio registers(D200-D208)
            lda #$0A				;
            sta DLISTL				;
            lda #$0F				;
            ...
            jsr L0A91_TTL_COL		;Loads colors in to the Playfield registers
            lda #$52				;
            clc						;
            adc RTCLOK				;RTCLOK = $52 ---NOT SURE YET??? Used for addressing???
            ldx #$03				;
            ldy #$1C				;
            jsr L11E2_DRW_SCN		;Load 'PLAYER 1' data to screen memory
            lda #$40				;
...
L0B77:      sta L013C,X			;$13C - $143 = 1
            ...
            dex					;
            bpl L0B77			;-----------------
            jsr L1E34_AUDIO		;Writes 0 to $D200 - $D208(AUDF1,AUDC1 - AUDF4,AUDC4), $8E - $91
            lda #$05
...
L0D7C:      .byte $0D,$A7,$0D,$C4,$0D,$CF,$0D,$DA,$0D,$F9,$0D,$40,$0E,$67,$0E,$6A
            ...
            .byte $C2,$FF,$00,$42,$FF
L0EF1_DL:   .byte $70,$70,$40	;Start screen display list
L0EF4:      .byte $20
L0EF5:      .byte $20
...
;Vertical blank
L100A_VBI2: cld				        ;Clear decimal mode
            ...
            jsr L066B_SET_COLOURS				;Loads first colors
...
;            
L11B9_SOUND: jsr L2579			;Possibly has something to do with players on the screen????
            ldx #$02			;Player index
...
L11E2_DRW_SCN: pha					;
            jsr L13B7_SET_RTCLOK			;Grab low and high byte from table based on X register
            pla					;
            asl					;A x2
            tax					;A -> X
            sty BUFADR			;
...
L1210:      jsr L1E34_AUDIO		;Clear audio registers
            lda L00F8			;Player alive or on screen?
            bne L121A			;Reset bruce
            jmp L046D_TITLES		;Jump to title screen
...
L121E:      lda #$24			;$24 fresh hit points
            ...
            sta L0088			;????
            sta L0089			;????
            jsr L1F40_PLAYER_SPRITE_SETUP			;Calculates Sprite positions and erases them
            jsr L38FF			;
            jsr L3E3E			;
            lda L398E			;
            beq L1245
...
L1267:      	lda L3A01,X
				...
				dex
				bpl L1267
L1278:      jsr L0568_INIT
            lda #$C0
            sta NMIEN
            ldx SCRNMBR
            jmp L299B
			
L1285:      jsr L1F40_PLAYER_SPRITE_SETUP
            jsr L38FF
            jsr L13A9_SCREEN_CLEAR
            lda #$0A
            sta DLISTL
            lda #$0F
            sta DLISTH
...
L1323:      lda FMSZPG+2		;
            ...
            jmp L0418_DOSVEC	;Jump to program reset
...
L133D:      lda FMSZPG+2		;
            ...
            jmp L046D_TITLES			;Jump to title screen loop
L1344:      rts

;Checks to see if game is paused and debounces keyboard
L1345_PAUSE: lda L00F9			;Load game paused status
            pha					;Save status
...
L135D_RESUM: lda L00F9			;Load pause status bit
            and #$06			;Mask of bits 1 and 2
...
L1395:      lda #$11			;
            ...
            jsr L13CC_1UP		;Store '1UP' in screen memory
            lda #$51			;
...
;			
L13A9_SCREEN_CLEAR:	ldx #$00			;
L13AB:      			jsr L13B7_SET_RTCLOK			;This subroutine moves the low and high byte addresses to $13, $14
...
L13CC_1UP:  dec L3A2C			;Wait $20 game loops
...
L13E9:      lda #$00			;
            tax					;
            ldy #$34			;
            jmp L11E2_DRW_SCN	;Erase '1 UP' from screen
...
L13FF:      ldx RTCLOK			;
            ...
            ldx #$01			;
            ldy #$04			;
            jmp L11E2_DRW_SCN	;Draw '1 UP' to screen memory
L140A:      rts

L140B:      lda #$03			;
            ldx #$01			;
            ldy #$10			;
            jmp L11E2_DRW_SCN	;Draw 'TOP' to screen memory
			
L1414:      lda #$05			;
            ldx #$01			;
            ldy #$1C			;
            jmp L11E2_DRW_SCN	;Draw 'FALLS' to screen memory
...
;Get keyboard state			
L1464_KYBD_RD:	lda SKSTAT			;$FF = No keys pressed $FB = other key press
...
L1508_MAIN: lda #$02			;
            ...
            jsr L1312_ST_BTN	;Check for Start button push during gameplay
            lda L00F8			;Is player on screen?
            beq L1523_NO_PSE	;Branch if player is not on screen
            jsr L1464_KYBD_RD	;Checks for keyboard input and sets related bits accordingly
            jsr L1345_PAUSE		;Checks to see if game is paused and debounces keyboard	
            jsr L135D_RESUM		;Checks if need to Pauses/resume game
            lda L00F9			;Load Pause status and keyboard debounce byte
            bpl L1523_NO_PSE	;Branch if not paused(%0xxxxxxx)
				jmp L159C_PAUSED	;Jump if paused
			
L1523_NO_PSE: lda NINJAHP			;
            and #$87			;%10000111 - Not sure yet?????
            sta NINJAHP			;
            jsr L13CC_1UP		;Draw and erase '1 UP' from screen   
            jsr L15A3_REDUNDANT	;Simply returns - should be removed - 3 NOP's
            jsr L37A4_RND		;Creates and stores a lot of random numbers
            lda DELTAC			;
            bne L153F			;
            jsr L0A07_TIMER_CHK	;Test Ninja and Yamo Timers
            jsr L0972_SET_YAMO	;Create instance of Yamo
            jsr L0A14_SET_NINJA	;Create instance of Ninja
L153F:      lda DELTAC+1		;
            bne L1549			;
            lda #$02			;
            sta DELTAC+1		;
            bne L154B			;
L1549:      dec DELTAC+1		;
L154B:      jsr L318F_SCR_NO	;Returns to an address based on screen number
            ...
            sta L00B8			;And here
            jsr L2634_LAMP_COL	;Lamp collisions and other stuff????
            jsr L2205_COL_INFO	;Sets collision info. and substracts points for collisions
            jsr L2926_SCORE		;Computes score and high score and store vales on screen
            jsr L11B9_SOUND		;Updates sounds and other stuff????	
            jsr L2532_LOW_PLR	;Calculates screen position of lowest player, saves is OLDROW
L1587:      lda VCOUNT			;
            cmp OLDROW			;Is drawing beam past lowest player position or screen position?
            bcc L1587			;Loop until it is
            jsr L1F40_PLAYER_SPRITE_SETUP			;Flip player status bit, erase players
            jsr L1E45_DRAW			;Draw players, store player color, size
            lda #$00			;
            sta HITCLR			;Clear Player/Missile collision registers
            jsr L2552_DEAD		;Checks to see if players have been killed
L159C_PAUSED: lda FMSZPG+2		;
            bne L159C_PAUSED	;Timer delay
            jmp L1508_MAIN		;Jump to Main Loop
L15A3_REDUNDANT: rts
...
L1DEA:      lda L0D6E			;
            bne L1E34_AUDIO		;Clear audio registers
            ldy #$03			;
L1DF1:      sty HOLD1			;
            lda L008E,Y			;
            beq L1E15			;
...
L1F40_PLAYER_SPRITE_SETUP:	ldx #$02				;Player index
...
;Opens wall room 2 if screen 6 retrieved, makes cows moo
            ...
            ldx #$0B			;
            ldy #$25			;
            jsr L11E2_DRW_SCN	;This copies Blank spaces to wall section
            lda #$07			;
            ldx #$0C			;
            ldy #$25			;
            jsr L11E2_DRW_SCN	;This copies Blank spaces to wall section
L2DC0:      lda DELTAC+1		;
            ...
            ldx #$09			;
            ldy #$21			;
            jsr L11E2_DRW_SCN	;Make cow's mouth open
            inc L427A			;
            bne L2DF2			;
L2DE1:      lda #$1E			;
            ldx #$09			;
...
L2FC3:      lda L4174
            ...
            ldx #$0B
            ldy #$00
            jsr L11E2_DRW_SCN	;Opens second door on screen 5, possibly a wall too????
...
L2FEE:      lda L427E
            ...
            ldx #$02
            ldy #$22
            jsr L11E2_DRW_SCN	;Opens walls on screen 5
            lda #$03
            ldy #$1E
            jsr L31AA
            lda #$06
...
L31EA:      ldx SAVADR+1		;---------------------------------
            ...
            jsr L11E2_DRW_SCN	;values passed in
            inc SAVADR+1		;---------------------------------
            rts
...
L3E67:      .byte $00,$64,$94
L3E6A_COLOURS:      .byte $32,$00,$0A,$00,$00
